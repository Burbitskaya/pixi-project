...
export class Player {
      ...
      if (newCol !== currentCol || newRow !== currentRow) {
        if (!this.tileMap.isWalkable(newCol, newRow)) {
          ...
          if (dx !== 0 && dy !== 0) {
            ...
            if (
              tryColX !== currentCol &&
              !this.tileMap.isWalkable(tryColX, currentRow)
            ) {
              newX = this.sprite.x;
            } else {
              newX = tryX;
            ...
            if (
              tryRowY !== currentRow &&
              !this.tileMap.isWalkable(currentCol, tryRowY)
            ) {
              newY = this.sprite.y;
            } else {
              newY = tryY;
  ...
  public getAttackCell(): { col: number; row: number } | null {
    ...
    if (
      col < 0 ||
      col >= this.tileMap.groundLayer.width ||
      row < 0 ||
      row >= this.tileMap.groundLayer.height
    ) {
      return null;
    }
